...
const ApplicationSummary = ({
  ...
  function getSummaryText() {
    if (application.status == 1)
      return 'The application has been submitted to the area manager for sign off.';
    if (application.status == 3)
      return 'The application has been accepted by the area manager and is now complete.';
  }
      ...
      {application.status == '2' && isRetailer && (
        <div className='flex justify-center'>
          <button
            ...
            className='bg-primary hover:opacity-75 text-white font-bold py-2 px-4 mb-4 rounded pointer'
          >
            Restart application
          </button>
        </div>
