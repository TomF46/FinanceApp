                    ...
                    <div className="my-8">
                        <div className="my-2 card shadow-md rounded-md">
                            <div className="bg-primary rounded-t-md">
                                <p className="text-white font-bold text-lg px-2 py-1">{user.fullName} Info</p>
                            </div>
                            <div className="px-2 py-1">
                                ...
                                <p><span className="font-bold">Role:</span> {user.roleTitle}</p>
                                {user.areasManaged.length > 0 && (
                                    <>
                                        <p className="font-bold">{`Area${user.areasManaged.length > 1 ? 's' : ''} managed:`}</p>
                                ...
                                {user.retailLocationsManaged.length > 0 && (
                                    <>
                                        <p className="font-bold">{`Retail location${user.retailLocationsManaged.length > 1 ? 's' : ''} managed:`}</p>
                                        {user.retailLocationsManaged.map((location) => {
                                            return (
                                                <p>{location.name} ({location.area})</p>
                                            )
                                        })}
                                    </>
                                )}
